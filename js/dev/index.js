...
function tabs() {
  ...
  function setTabsAction(e) {
    ...
    if (el.closest("[data-fls-tabs-title]")) {
      ...
      if (tabTitle.classList.contains("--tab-active") && isMobile) {
...
function initSliders() {
  const thumbsSwiper = new Swiper(".about__thumbs", {
    ...
    on: {
      init: function() {
        ...
        if (fractionEl) {
          fractionEl.innerHTML = `${this.realIndex + 1} / ${this.slides.length}`;
        }
  ...
  if (thumbsSwiper) {
    thumbsSwiper.slides.forEach((slide2, index) => {
      const num = String(index + 1).padStart(2, "0");
      const indexEl = document.createElement("span");
      indexEl.classList.add("thumb-index");
      indexEl.textContent = num;
      slide2.appendChild(indexEl);
    });
  }
...
document.querySelectorAll(".item-product").forEach((item) => {
  item.addEventListener("click", () => {
    if (item.classList.contains("active")) {
      ...
      document.querySelectorAll(".item-product.active").forEach((el) => {
